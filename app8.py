...
if raw_df is not None and len(raw_df) > 0:
    ...
    ).sort_values("emissions_reduction_tco2e", ascending=False)
    ...
    with eff_chart_col1:
        st.markdown("**Efficiency emissions reduction by commodity**")
        st.bar_chart(eff_by_commodity.set_index("commodity")[["emissions_reduction_tco2e"]])
    ...
    st.markdown("**Top efficiency opportunity sites**")
    top_eff_sites = eff_by_site.head(15).copy()
    top_eff_sites["site_label"] = top_eff_sites["site_name"] + " | " + top_eff_sites["state"]
    st.bar_chart(top_eff_sites.set_index("site_label")[["emissions_reduction_tco2e"]])
