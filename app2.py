...
COLUMN_ALIASES = {
    ...
    "unit": [
        "unit", "uom", "units", "usage_unit", "quantity_unit"
    ],
    "annual_cost": [
...
OPTIONAL_CANONICAL_COLUMNS = [
    "charge_type", "account_number", "meter_number", "period_start", "period_end"
]
...
def standardize_uploaded_data(raw_df, mapping):
    df = pd.DataFrame()
    ...
    df["site_name"] = df["site_name"].fillna("Unknown Site").astype(str).str.strip()
    df["state"] = df["state"].apply(normalize_state)
    df["commodity"] = df["commodity"].apply(normalize_commodity)
...
if raw_df is not None and len(raw_df) > 0:
    ...
    missing_required = [col for col in REQUIRED_CANONICAL_COLUMNS if mapping.get(col) is None]
    ...
    standardized_df = standardize_uploaded_data(raw_df, mapping)
